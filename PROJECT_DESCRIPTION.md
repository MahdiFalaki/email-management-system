...
## Local test steps (Module 1: Schedules)

Use these exact steps to validate the new Schedules page behavior locally.
...
3. **Create test data for schedules**
   - Open `📧 Send Email` page.
   - Ensure at least one profile and one template exist:
     - if missing, create from `👥 Profiles` and `📄 Email Templates` pages.
   - In `📧 Send Email`:
...
4. **Validate Schedules page rendering**
   - Open `📅 Schedules` page.
...
5. **Validate reschedule action**
   - For any schedule, change date/time and click `💾 Reschedule`.
   - Expected result:
     - success message appears,
     - page refreshes,
     - new datetime is shown.

6. **Validate past-time guardrail**
   - Try setting an earlier/past datetime and click `💾 Reschedule`.
   - Expected result:
     - error message: scheduled time must be in the future,
     - schedule should not update.

7. **Validate cancel action**
   - Click `🗑️ Cancel` on one schedule.
   - Expected result:
     - success message appears,
     - page refreshes,
     - schedule disappears from list.

8. **Optional DB-level verification**
   - Inspect `email_manager.json` and confirm changes in `schedules` table after reschedule/cancel actions.

### Debug checklist (if anything fails)
- If Schedules page is empty, verify a schedule was created from `📧 Send Email`.
- If subject/recipients are missing, linked `sent_emails` entry may be missing or stale.
- If reschedule does not persist, check TinyDB file write permissions and rerun app.
- If app fails to start, run `python -m compileall src` and verify dependencies are installed.
